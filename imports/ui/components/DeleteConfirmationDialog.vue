<template>
  <v-dialog v-model="isVisible" persistent :max-width="getMaxWidth">
    <v-card :color="getCardColor">
      ...
      <v-card-actions v-if="actionFailed"> <v-btn color="secondary" @click.native="closeDialog" class="mx-auto"> OK </v-btn> </v-card-actions>
...
export default {
  ...
  methods: {
    ...
    deleteItem() {
      Meteor.call(this.$router.currentRoute.meta.methodPrefix.concat(".remove"), this.mongoId, error => {
        if (error) {
